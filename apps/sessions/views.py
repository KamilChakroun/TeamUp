...
from .models import Session, Invitation,ChatRoom,MessageReadReceipt,Message,BlockedUser
...
def chat_room(request, session_id):
    """Afficher le chat d'une session"""
    session = get_object_or_404(Session, id=session_id)
    
    # Vérifier que l'utilisateur a accès
    if not (request.user == session.creator or 
            session.invitees.filter(id=request.user.id, invitation__status='accepted').exists()):
        return redirect('sessions:detail', pk=session_id)
    
    # Créer ou récupérer le chat room
    chat_room, created = ChatRoom.objects.get_or_create(session=session)
    ...
    # Récupérer tous les messages non supprimés
    ...
    accepted_invitees = session.invitees.filter(invitation__status='accepted')
    participants_list = [session.creator] + list(accepted_invitees)
    
    # Préparer les données des participants avec statut de blocage
    participants = []
    ...
    return render(request, 'sessions/chat_room.html', context)
...
def send_message(request, session_id):
    ...
    # Créer ou récupérer le chat room
    chat_room, _ = ChatRoom.objects.get_or_create(session=session)
...
def chat_list(request):
    """Liste des sessions avec chat disponibles"""
    # Sessions où l'utilisateur est créateur ou invité accepté
    user_sessions = Session.objects.filter(
        Q(creator=request.user) | 
        Q(invitees=request.user, invitation__status='accepted')
    ).distinct().annotate(
        message_count=Count('chat_room__messages', filter=Q(chat_room__messages__is_deleted=False)),
        last_message_time=Max('chat_room__messages__created_at')
    ).order_by('-last_message_time')
    
    context = {
        'sessions': user_sessions
    }
    return render(request, 'sessions/chat_list.html', context)
...
def get_messages(request, session_id):
    ...
    chat_room = get_object_or_404(ChatRoom, session=session)
    ...
    # Récupérer les nouveaux messages (non bloqués, non supprimés)
...
def block_user(request, user_id):
    ...
    return JsonResponse({
        'success': True,
        'message': f'{user_to_block.username} a été bloqué avec succès'
    })
...
def unblock_user(request, user_id):
    ...
    return JsonResponse({
        'success': True,
        'message': f'{user_to_unblock.username} a été débloqué avec succès'
    })
...
def blocked_users_list(request):
    ...
    return render(request, 'sessions/blocked_users_list.html', context)
